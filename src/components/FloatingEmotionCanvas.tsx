  ...
  return (
    <div className="relative w-full h-screen overflow-hidden bg-[#5a0000]">
      ...
      {orbs.map((orb, index) => (
        <div
          key={orb.id || index}
          className="absolute rounded-full text-white text-xs p-4 flex items-center justify-center text-center overflow-hidden break-words whitespace-pre-wrap hover:scale-105 transition-transform duration-200"
          style={{
            left: orb.x,
            top: orb.y,
            width: orb.size,
        ...
        >
          <span className="text-sm leading-snug">{orb.preview}</span>
        </div>
      ))}
